...
import '../main.dart'; // للوصول لـ SplashScreen
        ...
        children: [
          ...
          _buildNavTilesGroup([
            ...
            _NavTile(
              ...
              iconBg: const Color(0xFFEF4444),
              title: 'إعادة ضبط بيانات الجهاز (تجربة أول تشغيل)',
              subtitle: 'مسح التفعيل وبيانات السحابة للبدء من الصفر',
              onTap: _resetDeviceData,
              isDanger: true,
            ),
          ]),
          const SizedBox(height: 20),
  ...
  Future<void> _resetDeviceData() async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          'سيتم مسح بيانات التفعيل وربط السحابة وأي جلسات دخول من هذا الجهاز فقط.\n\n⚠️ هذا سيعيد التطبيق لحالة "أول تشغيل" لاختبار تجربة الإعداد من البداية.',
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () => Navigator.pop(ctx, true),
            child: const Text('نعم، إعادة الضبط', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
          ),
        ],
      ),
    );
    ...
    if (confirm == true) {
      try {
        await DatabaseHelper.instance.resetDeviceAndActivationData();
        if (!mounted) return;

        context.read<CurrentUserProvider>().loginAsOwner();

        showSnack(context, 'تمت إعادة ضبط بيانات الجهاز بنجاح 🔄');

        // التوجيه فوراً إلى شاشة البداية لتجربة أول تشغيل نظيفة
        ...
        if (mounted) {
          showSnack(context, 'حدث خطأ أثناء إعادة الضبط: $e', isError: true);
        }
      }
    }
  }
