...
class _DevPanelScreenState extends State<DevPanelScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadCodes() async {
    final db = await DatabaseHelper.instance.database;
    final codes = await db.query('subscription_codes', orderBy: 'created_at DESC');
            ...
            child: Column(
              ...
              children: [
                ...
                Row(children: [
                  ...
                  const SizedBox(width: 8),
                  Expanded(child: AppTextField(hint: 'خصم %', controller: discountCtrl, keyboardType: TextInputType.number)),
                  const SizedBox(width: 8),
                  Expanded(child: AppTextField(hint: 'عدد مرات الاستخدام', controller: maxUsesCtrl, keyboardType: TextInputType.number)),
                ]),
                const SizedBox(height: 10),
                const Text('الباقة', style: TextStyle(color: AppColors.textMuted, fontSize: 12)),
                ...
                Wrap(spacing: 8, children: [
                  for (final p in ['basic', 'pro', 'premium'])
                    ChoiceChip(
                      ...
                      labelStyle: TextStyle(color: plan == p ? Colors.white : AppColors.textMuted, fontSize: 12),
                      side: BorderSide(color: plan == p ? AppColors.primary : AppColors.darkBorder),
                    ),
                ]),
                const SizedBox(height: 16),
                PrimaryButton(
        ...
        builder: (ctx, setBS) => Padding(
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('📢 إضافة إعلان', style: TextStyle(color: AppColors.primary, fontWeight: FontWeight.w700, fontSize: 16)),
                const SizedBox(height: 16),
  ...
  Widget _buildLogin() {
    return Scaffold(
      backgroundColor: AppColors.dark,
      appBar: AppBar(
        title: const Text('لوحة المطور 🔐', style: TextStyle(fontWeight: FontWeight.w700)),
        backgroundColor: AppColors.darkCard,
        leading: IconButton(
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            const Text('الدخول مقصور على المطور فقط', style: TextStyle(color: AppColors.textColor, fontSize: 18, fontWeight: FontWeight.w700)),
            const SizedBox(height: 8),
            const Text('أدخل كلمة مرور المطور', style: TextStyle(color: AppColors.textMuted)),
  ...
  Widget _buildPanel() {
    return Scaffold(
      backgroundColor: AppColors.dark,
      appBar: AppBar(
        title: const Text('لوحة المطور 🛠️', style: TextStyle(fontWeight: FontWeight.w700)),
        backgroundColor: AppColors.darkCard,
        leading: IconButton(
          icon: const Icon(Icons.arrow_back_ios_rounded, color: AppColors.textColor),
          onPressed: () => Navigator.pop(context),
    ...
    return Column(
      children: [
        Padding(
          ...
          child: PrimaryButton(text: '➕ إضافة كود جديد', onTap: _addCode, icon: Icons.add),
        ),
        Expanded(
          child: _codes.isEmpty
              ? const EmptyState(emoji: '🎟️', title: 'لا توجد أكواد', subtitle: 'أضف أكواد الاشتراك والخصم')
              : ListView.builder(
                  ...
                  itemBuilder: (ctx, i) {
                    ...
                    return Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(color: isActive ? AppColors.primary.withValues(alpha: 0.3) : AppColors.darkBorder),
                      ),
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Row(
                            children: [
                              Expanded(
                                child: Text(c['code'], style: const TextStyle(color: AppColors.primary, fontWeight: FontWeight.w800, fontSize: 16, letterSpacing: 2)),
                              ),
                              Container(
                                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                                decoration: BoxDecoration(
                                  color: isActive ? AppColors.primaryLight : const Color(0xFFFEE2E2),
                                  borderRadius: BorderRadius.circular(20),
                                ),
                                child: Text(isActive ? 'فعال' : 'منتهي',
                                    style: TextStyle(color: isActive ? AppColors.primary : AppColors.danger, fontSize: 11, fontWeight: FontWeight.w700)),
                              ),
                            ],
                          ),
                          const SizedBox(height: 8),
                          Wrap(
                            ...
                            children: [
                              ...
                              _codeInfo('الاستخدام', '${c['used_count']}/${c['max_uses']}'),
                            ],
                          ),
                        ],
                      ),
  ...
  Widget _codeInfo(String label, String value) => Row(
    ...
    children: [
      ...
      Text(value, style: const TextStyle(color: AppColors.textLight, fontSize: 12, fontWeight: FontWeight.w600)),
    ],
  );
  ...
  Widget _buildAdsTab() {
    return Column(
      children: [
        Padding(
          padding: const EdgeInsets.all(16),
          child: PrimaryButton(text: '📢 إضافة إعلان جديد', onTap: _addAd, icon: Icons.campaign),
        ),
        Expanded(
          child: _ads.isEmpty
              ? const EmptyState(emoji: '📢', title: 'لا توجد إعلانات', subtitle: 'أضف إعلانات لعرضها للمستخدمين')
              : ListView.builder(
                  padding: const EdgeInsets.fromLTRB(16, 0, 16, 16),
                        ...
                        children: [
                          Expanded(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text(ad['title'], style: const TextStyle(color: AppColors.textColor, fontWeight: FontWeight.w700)),
                                Text(ad['body'], style: const TextStyle(color: AppColors.textMuted, fontSize: 12), maxLines: 2, overflow: TextOverflow.ellipsis),
                                Text('الشاشة: ${ad['screen']}', style: const TextStyle(color: AppColors.primary, fontSize: 11)),
                              ],
                            ),
                          ),
                          Switch(
      ...
      builder: (ctx, snap) {
        ...
        return ListView(
          ...
          children: [
            ...
            _statRow('🎟️ أكواد مفعلة', '${_codes.where((c) => c['is_active'] == 1).length}'),
            _statRow('📢 إعلانات نشطة', '${_ads.where((a) => a['is_active'] == 1).length}'),
          ],
        );
      },
    );
  ...
  Widget _statRow(String label, String value) => Container(
    ...
    child: Row(
      ...
      children: [
        ...
        Text(value, style: const TextStyle(color: AppColors.primary, fontWeight: FontWeight.w800, fontSize: 18)),
      ],
    ),
  );
}
